...
export const SleepTipsScreen: React.FC = () => {
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      const sleepTips = dailyTips?.sleep || [];

      if (sleepTips.length > 0) {
        setRiskLevel(sleepTips[0].riskLevel?.toLowerCase() || 'low');
      } else {
        setRiskLevel('low');
      }

      setRecommendations(sleepTips);
    } catch (error) {
      console.error('Error loading sleep recommendations:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <View style={styles.mainCard}>
          <View style={styles.cardHeader}>
            ...
            <View style={styles.cardTitleContainer}>
              <Text style={styles.cardTitle}>Sleep & Lifestyle Tips</Text>
          ...
          {loading ? (
            ...
            <View style={styles.tipsContainer}>
              {recommendations.map((tip, index) => (
                <View
                  key={tip.id ?? `${tip.category || 'sleep'}-${index}`}
                  style={[styles.tipCardColored, { backgroundColor: '#E3F2FD' }]}
                >
            ...
            <View style={{ padding: 40, alignItems: 'center' }}>
              ...
              <Text style={{ marginTop: 12, color: COLORS.textSecondary, textAlign: 'center' }}>
                No lifestyle recommendations available for your current risk level.
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={{ 
          ...
          <Text style={{ color: COLORS.textSecondary, fontSize: 13, lineHeight: 20 }}>
            These lifestyle and sleep recommendations are personalized based on your latest Jendo test results. 
            Good sleep hygiene is essential for cardiovascular health.
          </Text>
        </View>
      </ScrollView>
    </ScreenWrapper>
