...
export const ExerciseTipsScreen: React.FC = () => {
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      const exerciseTips = dailyTips?.exercise || [];

      if (exerciseTips.length > 0) {
        setRiskLevel(exerciseTips[0].riskLevel?.toLowerCase() || 'low');
      } else {
        setRiskLevel('low');
      }

      setRecommendations(exerciseTips);
    } catch (error) {
      ...
      >
        <View style={styles.mainCard}>
          <View style={styles.cardHeader}>
            <View style={[styles.iconContainer, { backgroundColor: getRiskBgColor(riskLevel) }]}>
              <MaterialCommunityIcons name="dumbbell" size={28} color={getRiskColor(riskLevel)} />
            </View>
          ...
          {loading ? (
            ...
            <View style={styles.tipsContainer}>
              {recommendations.map((tip, index) => (
                <View
                  key={tip.id ?? `${tip.category || 'exercise'}-${index}`}
            ...
            <View style={{ padding: 40, alignItems: 'center' }}>
              ...
              <Text style={{ marginTop: 12, color: COLORS.textSecondary, textAlign: 'center' }}>
                No exercise recommendations available for your current risk level.
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={{ 
          ...
          <Text style={{ color: COLORS.textSecondary, fontSize: 13, lineHeight: 20 }}>
            These exercise recommendations are personalized based on your latest Jendo test results. 
            Regular physical activity can help improve your cardiovascular health.
          </Text>
        </View>
      </ScrollView>
    </ScreenWrapper>
